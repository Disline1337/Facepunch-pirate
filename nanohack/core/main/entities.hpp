...
/*				if (plusminus::ui::get_bool(xorstr_("debug")) && entity->transform()->position().distance(LocalPlayer::Entity()->transform()->position()) <= 5.0f) {
				...
				if (entity->class_name_hash() == STATIC_CRC32("BasePlayer") ||
					...
					if (!bounds.empty()) {
						...
						if (plusminus::ui::get_bool(xorstr_("name"))) {
							Renderer::text(headPos, col, 12.f, true, true, wxorstr_(L"%s [%dhp]"), player->_displayName(), (int)ceil(player->health()));
						}
						
						if (plusminus::ui::get_bool(xorstr_("dist"))) {
							Renderer::text(headPos, col, 12.f, true, true, wxorstr_(L"%s [%dhp] [%.2fm]"), player->_displayName(), (int)ceil(player->health()), player->bones()->head->position.distance(local->bones()->head->position));
						}
				...
				if (entity->class_name_hash() == STATIC_CRC32("BaseHelicopter") && plusminus::ui::get_bool(xorstr_("helicopter"))) {
					if (entity->ShortPrefabName_hash() == STATIC_CRC32("patrolhelicopter")) {
						Vector2 screen;
						if (Camera::world_to_screen(entity->transform()->position(), screen)) {
							switch (entity->ShortPrefabName_hash()) {
							case STATIC_CRC32("patrolhelicopter"):
								Renderer::text(screen, plusminus::ui::get_color(xorstr_("patrolheli")), 12.f, true, true, wxorstr_(L"patrol helicopter"));
								break;
							}
						}
					}
				}
				if (entity->class_name_hash() == STATIC_CRC32("CollectibleEntity") && plusminus::ui::get_bool(xorstr_("collectable"))) {
					if (entity->ShortPrefabName_hash() == STATIC_CRC32("stone-collectable") ||
						entity->ShortPrefabName_hash() == STATIC_CRC32("sulfur-collectable") ||
						entity->ShortPrefabName_hash() == STATIC_CRC32("metal-collectable")) {
						Vector2 screen;
						if (Camera::world_to_screen(entity->transform()->position(), screen)) {
							switch (entity->ShortPrefabName_hash()) {
							case STATIC_CRC32("stone-collectable"):
								Renderer::text(screen, plusminus::ui::get_color(xorstr_("ores collectable")), 12.f, true, true, wxorstr_(L"Stone collectable"));
								break;
							case STATIC_CRC32("sulfur-collectable"):
								Renderer::text(screen, plusminus::ui::get_color(xorstr_("ores collectable")), 12.f, true, true, wxorstr_(L"Sulfur collectable"));
								break;
							case STATIC_CRC32("metal-collectable"):
								Renderer::text(screen, plusminus::ui::get_color(xorstr_("ores collectable")), 12.f, true, true, wxorstr_(L"Metal collectable"));
								break;
							}
						}
					}
				...
				if ((entity->class_name_hash( ) == STATIC_CRC32("PlayerCorpse") ||
					...
					entity->class_name_hash( ) == STATIC_CRC32("DroppedItemContainer") && plusminus::ui::get_bool(xorstr_("corpses"))) {
					Vector2 screen;
					if (Camera::world_to_screen(entity->transform( )->position( ), screen)) {
