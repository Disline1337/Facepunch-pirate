...
void OnAttacked_hk(BaseCombatEntity* self, HitInfo* info) {
	if (plusminus::ui::get_bool(xorstr_("hitsound"))) {
		...
		if (self->class_name_hash() == STATIC_CRC32("BasePlayer")) {
			if (info->Initiator() == LocalPlayer::Entity()) {
				auto entity = reinterpret_cast<BasePlayer*>(self);
					...
					if (std::filesystem::exists(settings::data_dir + xorstr_("\\hitsound.wav")))
						play_sound((settings::data_dir + xorstr_("\\hitsound.wav")).c_str());
			}
		}
	}
}
void ClientInput_hk(BasePlayer* plly, uintptr_t state) {
	...
	if (plly->userID( ) == LocalPlayer::Entity( )->userID( )) {
		...
		if (plusminus::ui::get_bool("long neck") && get_key(plusminus::ui::get_keybind("long neck bind"))) {
	...
	if (plusminus::ui::get_bool(xorstr_("spider"))) {
		LocalPlayer::Entity()->movement()->groundAngle() = 0.f;
		LocalPlayer::Entity()->movement()->groundAngleNew() = 0.f;
	}
	if (plusminus::ui::get_bool(xorstr_("gravity")))
	{
		LocalPlayer::Entity()->movement()->gravityMultiplier() = 9.f;
	}
}
